...
- Transitive-execution shapes invisible to code-execution analysis: `_segment_runs_code` recognized interpreters with a script-extension argument and package tools, but missed a bare path-qualified executable (`./scripts/setup.sh`, `/opt/x/run`), an interpreter given a path argument with no script extension (`sh ./install`, this class's own canonical example), and container builds or runs (`docker build`, `docker run`, `docker compose up`) that execute an in-repo `Dockerfile` or a container entrypoint. Those actions were not classified `code-executing`, so an author could understate them as `class: normal` and the verifier surfaced nothing. The classifier now treats a `./`, `../`, or `/`-prefixed executable, a script-extension program name, an interpreter with a path-qualified positional, and `docker`/`podman`/`buildah`/`nerdctl` `build`/`bud`/`run`/`exec`/`start` (and `compose up`/`run`/`build`) as code-executing, so under-declaration raises `action-block.class.code-executing-missing` and `approval.command-implies-required`. Read-only container queries (`docker ps`) and data tools (`sed`, `jq`) are guarded against false positives. Additive detection reusing existing warning ids; regression coverage in `scripts/test_parser_edge_cases.py` and fixture `fixtures/invalid/command-local-script-under-declared`. This is the detection substrate the 0.7.0 `action.exec-unbounded` rule keys off (defined in `spec.md` section 12, "Code-executing actions and the review boundary"); it does not itself add the bounded-artifact requirement or change the profile version.
