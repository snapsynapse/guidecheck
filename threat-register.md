...
- Verifier fingerprinting / targeted cloaking: the content-variation re-fetch no longer brands itself as GuideCheck, but the verifier remains identifiable by source-IP range, the back-to-back two-fetch pattern from one IP (the first fetch is still branded), `Accept-Encoding: identity`, and the distinctive `Accept` header. A host that fingerprints the verifier can serve benign bytes to it and malicious bytes to agents. Stronger mitigations (a third randomized probe, a credible real-agent header profile, fetching from a distinct egress) are deferred. Level 5 agent byte-agreement (the agent compares its own fetched bytes against the verifier-reported hash) remains the durable defense; lower levels cannot fully close this.
- Hosted verifier as request proxy: one request can drive up to ~5 outbound fetches to publisher-declared URLs (guide, content-variation refetch, manifest, registry, transparency log), enabling modest amplification and source-IP laundering against arbitrary public hosts. SSRF-to-private is blocked. The in-memory per-warm-instance rate limit is best-effort only; a durable or edge/WAF limit is recommended for production.
- Anchor hash extraction: `find_json_sha256` returns the first `sha256`-shaped value in a registry record rather than the `assistantGuide` field specifically, which can pick a decoy on real registry JSON and cause a false `anchor.independent.mismatch`. Deferred fix: key on the assistant-guide field and flag ambiguity.
- Spec/implementation tension: the local-file CLI can reach Level 4 from operator-supplied manifest and anchor files without any fetch, which the spec (section 6) forbids for a fetching verifier. Treated as trusted operator input; if formalized, the spec should carve out an explicit operator-attested Level 4 distinct from fetched Level 4.

### Non-security gaps noted

- Engine duplication: `scripts/guidecheck_verify.py` and `scripts/eval_guidecheck.py` carry hand-synced copies of the entire detection engine. Intended as an independent cross-check, it is in practice copy-paste; every fix in this review had to be applied twice. Either make the eval import the engine (and drop the independence claim) or make it genuinely independent.
- Version-string duplication: the `0.3.2` version appears in several constants across three files; centralize before the next bump.
- Finding-id documentation drift: six emitted ids were undocumented in `finding-ids.md` until this review. The contract validator now enforces documentation for any id referenced by a fixture; consider extending it to every emitted id.
