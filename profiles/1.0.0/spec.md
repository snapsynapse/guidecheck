...
## 28. Profile selection and compatibility
...
software MUST NOT silently re-evaluate a legacy guide as 1.0.0. Supported release
...
A 1.0.0 manifest MUST declare exactly one `profile` and `profile-version` matching
...
Legacy conformance is not 1.0.0 conformance. The repository's published self-guide
and existing anchors need not change when a new verifier or profile is released.
See [dispatch and output requirements](verifier-conformance.md#28-version-aware-dispatch-and-reports).
