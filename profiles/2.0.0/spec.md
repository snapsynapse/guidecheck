...
29. [Profile Selection and Compatibility](#29-profile-selection-and-compatibility)
...
The channel rules below distinguish qualifying independent provenance from corroboration. A publisher MAY use more than one channel. The verifier MUST cross-check every channel it can find and MUST emit a failure when channels disagree on the hash.
...
A public repository file at `source-path` provides corroborating hash evidence, not a qualifying independent anchor in profile 2.0.0. The verifier MUST compare the served bytes with the repository file and MUST retain the observed match or mismatch. Administrative separation is unestablished by the supported repository fetch evidence, including when the guide has a custom domain, a different hosting vendor, or a commit-pinned source URL. Repository-file evidence MUST NOT satisfy the Level 4 independent-anchor requirement, even when another channel qualifies. A repository hash mismatch MUST block Level 4. A future positive independence exception requires a separately specified evidence protocol; publisher assertions and URL shape are insufficient.
...
- Unresolved: a code-executing invocation whose effective target ownership
  cannot be established from the command. Examples include `python -m` when
  module ownership is unknown, Cargo or Go builds whose hook status requires
  repository inspection, remote Docker contexts, `npm exec`, and every `npx`
  form. Package selection alone does not prove which binary `npx` executes.

Profile 2.0.0 treats unresolved execution as a conformance failure at Level 3.
The guide MUST identify a syntax-proven bound target and pin or inline it, or
use an established exempt dependency command. A declared `exec-sha256` cannot
resolve an unidentified target because the verifier cannot establish which
effective bytes the hash is intended to bind.

The following forms establish bound dispatch without repository inspection:

- `npm run <script>` and `npm test`, including a recognized `--prefix` that
  determines the effective `cwd` and `package.json`
- `make` or `make <target>`, including recognized `-C` and `-f` forms that
  determine the effective directory and makefile
- `just` or `just <recipe>`, including recognized `--justfile` forms that
  determine the effective justfile
- `pip install .`, `pip install -e .`, or another explicit local path
- `go generate`
- `docker build` with an explicit local context or local Dockerfile

Help and version queries do not dispatch repository code. An unrecognized
wrapper or dispatch option is unresolved rather than clean. When one command
contains multiple segments, any unresolved segment keeps the whole action
unresolved even if another segment names a pinnable target.
...
A verifier MUST flag occurrences of these patterns. It MUST evaluate each
occurrence independently so that a prohibition earlier in a sentence or prose
unit cannot consume or suppress a later affirmative instruction.

A negation suppresses a prohibited-pattern finding only when it directly
governs the matched instruction. One ordinary prose line wrap may occur between
the negation and governed phrase. Blank lines, sentence punctuation, semicolons,
metadata or action markers, structured fields, list items, and headings end the
scope. A comma-coordinated prohibition qualifies only when its narrow verb list
contains an explicit `and` or `or`. An intervening subject, contrast, temporal
or conditional term, unrelated reassurance, or more than one prose wrap ends
the scope. Ambiguous grammar does not suppress a finding.
...
- the `guide-sha256` MUST be cross-published on at least one independent control plane as defined in section 11 (DNS TXT, package registry metadata, signed security.txt, or public append-only transparency log)
...
## 29. Profile Selection and Compatibility

The guide profile declaration selects the evaluation contract. Verifiers supporting
legacy profiles MUST retain those profiles' anchor semantics. Upgrading verifier
software MUST NOT silently re-evaluate a legacy guide as 2.0.0. Supported release
versions are explicit; ranges and unknown or prerelease versions MUST NOT select
a nearby supported release. Syntactically valid build metadata does not change
the selected profile policy.

A 2.0.0 manifest MUST declare exactly one `profile` and `profile-version` matching
the guide's profile policy. Missing, duplicate, or conflicting manifest selectors
MUST block Level 4. The guide selects policy; a manifest cannot override it.
Consumers MAY require an exact supported profile policy. A verifier MUST reject
a guide that does not meet that requirement rather than reinterpret its bytes.

Legacy conformance is not 2.0.0 conformance. The repository's published self-guide
and existing anchors need not change when a new verifier or profile is released.
See [dispatch and output requirements](verifier-conformance.md#32-version-aware-dispatch-and-reports).
